			...
			if ( ! empty( $_POST['user_map'][$i] ) ) {
				...
				if ( ! is_wp_error( $user_id ) ) {
					...
					if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ){
						$err = sprintf( __( 'Failed to create new user for %s. Their posts will be attributed to the current user.', 'wordpress-importer' ), esc_html($this->authors[$old_login]['author_display_name']) );
						$this->logerror($err);
					}
				}
			}
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ){
					$err = sprintf(__( 'Failed to import category %s', 'wordpress-importer' ), esc_html($cat['category_nicename']) );
					$this->logerror($err . ':' . $id->get_error_message());
				}
				continue;
			}
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ){
					$err = sprintf( __( 'Failed to import %s %s', 'wordpress-importer' ), esc_html($term['term_taxonomy']), esc_html($term['term_name']) );
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ){
					$err = sprintf( __( 'Failed to import &#8220;%s&#8221;: Invalid post type %s', 'wordpress-importer' ),
			...
			$this->logerror(json_encode($post));
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ){
					$err = sprintf( __('%s &#8220;%s&#8221; already exists.', 'wordpress-importer'), $post_type_object->labels->singular_name, esc_html($post['post_title']) );
					$this->logerror($err);
				}
				...
				if ( is_wp_error( $post_id ) ) {
					if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ){
						$err = sprintf( __( 'Failed to import %s &#8220;%s&#8221;', 'wordpress-importer' ),
							$post_type_object->labels->singular_name, esc_html($post['post_title']) );
						
						$this->logerror($err . ':' . $post_id->get_error_message());
					}
					continue;
				}
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG ){
								$err = sprintf( __( 'Failed to import %s %s', 'wordpress-importer' ), esc_html($taxonomy), esc_html($term['name']) );
